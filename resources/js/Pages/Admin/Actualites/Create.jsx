...
export default function Create() {
    ...
    return (
        <AdminLayout user={auth.user}>
            <Head title="Ajouter une actualité - LIXUS ADMIN" />

            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Ajouter une publication</h1>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden p-6 max-w-4xl">
                <form onSubmit={submit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Images / Vidéos (Première sera la couverture)</label>
                            <input 
                                type="file" 
                                accept="image/*,video/*"
                                multiple
                                ...
                                <div className="mt-4 grid grid-cols-3 gap-2">
                                    {data.images.map((file, index) => (
                                        ...
                                        >
                                            ...
                                            {index === 0 && <span className="absolute bottom-1 left-1 bg-lixus-red text-white text-[10px] px-2 py-0.5 rounded">Couverture</span>}
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="pt-6 border-t border-slate-200 flex flex-col items-end">
                        ...
                        >
                            {processing ? 'Enregistrement...' : 'Créer l\'actualité'}
                        </button>
                    </div>
                </form>
            </div>
