...
export default function Edit({ salle }) {
    const { auth } = usePage().props;
    const [preview, setPreview] = useState(salle.image || null);
    ...
    const [galleryPreviews, setGalleryPreviews] = useState(() => {
        if (salle.images_salle && Array.isArray(salle.images_salle)) {
            return salle.images_salle.map(img => img.startsWith('http') || img.startsWith('/storage') ? img : '/storage/' + img);
        }
        return [];
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.salles.update', salle.id));
    };

    return (
        <AdminLayout user={auth.user}>
            <Head title={`Éditer ${salle.nom_salle} - LIXUS ADMIN`} />
            ...
            <div className="mb-8 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Éditer la salle</h1>
                    <p className="text-slate-500 mt-1 text-sm">Modifiez les informations de <strong>{salle.nom_salle}</strong>.</p>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full border border-slate-300 rounded-lg p-2.5 focus:ring-lixus-red focus:border-lixus-red"
                                placeholder="Vidéoprojecteur, Sonorisation..."
                            />
